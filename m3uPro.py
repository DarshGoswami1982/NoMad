...
def yaz(kullanici): 
    ...
    if not os.path.exists(hits_dir):
        ...
        print(f"Created directory: {hits_dir}")
    filename = hits_dir+fx+' 𝐦𝟑𝐮𝐏𝐫𝐨.txt'
    print(f"Writing to file: {filename}")
    dosya=open(filename,'a+', encoding='utf-8') 
    dosya.write(kullanici) 
    dosya.close()
    print("Hit saved to file!") 
cpm=0
proxy_="\33[1;33m"
            ...
            while True:
                try:
                    if Pro == 'y':
                        ...
                        res = ses.get(link, headers=HEADERd, timeout=1,
                                verify=False, proxies=PP)
